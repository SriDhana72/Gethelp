...
// Note: We removed 'const iframe' from here because we will recreate it dynamically
...
<body>
    <div id="chat-container">
        <div class="msg bot">Hello! I am your AI Assistant. How can I help you?</div>
    </div>
    <div class="typing" id="typing-indicator">Agent is typing...</div>
    <div id="input-area">
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            try {
                /* Need to UNCOMMENT THIS BLOCK WHEN we HAVE THE TOKEN */
                /*
                ...
                */
                ...
                // MOCK REPLY (DELETE THIS LATER)
                await new Promise(r => setTimeout(r, 1000));
                addMessage("Simulated Reply: I received '" + text + "'. (Add Token to make me real!)", 'bot');
...
// Helper to get a fresh iframe
function resetIframe() {
    // 1. Find existing iframe
    const oldIframe = document.getElementById('crmIframe');
    if (oldIframe) {
        oldIframe.remove(); // Completely delete it from DOM
    }
...
function openOption(type) {
    ...
    if (type === 'Live chat') {
        // DESTROY OLD & CREATE FRESH IFRAME
        const iframe = resetIframe();
        
        // Inject Content into fresh iframe
        ...
        // LOAD URL
        const iframe = resetIframe(); // Also reset for GC to ensure clean state
...
function closeCrmWindow() {
    ...
    // Optional: Clear iframe to save memory
